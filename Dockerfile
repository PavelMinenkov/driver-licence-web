...
CMD echo "Wait database is ready" && /app/wait-for-it.sh ${DATABASE_HOST}:${DATABASE_PORT-5432} --timeout=60 && \
  ...
  echo "Run services" && (python manage.py run_ssi_police & python manage.py run_ssi_carsharing & python manage.py run_ssi_gov & python manage.py run_ssi_driving_school & gunicorn settings.asgi:application --workers 4 --worker-class uvicorn.workers.UvicornWorker --bind 0.0.0.0:80)
